...
public class Search {
  ...
  private static final String KEYWORD_SQL = "SELECT DISTINCT keywords.id FROM keywords WHERE keywords.name LIKE ?";
  ...
  private static final String ALL_TAG_KEYWORD_ID_SQL = "SELECT DISTINCT tags_keywords_search.id FROM tags_keywords_search WHERE tags_keywords_search.tag_id = ?";

  /** The Constant ALL_TAG_KEYWORD_SQL. */
  private static final String TAG_KEYWORD_ID_SQL = ALL_TAG_KEYWORD_ID_SQL
      + " AND tags_keywords_search.keyword_id = ANY(?::int[])";

  // private static final String SEARCH_SQL =
  // "SELECT DISTINCT search_samples.sample_id FROM search_samples WHERE
  // search_samples.search_field_keyword_id = ?";

  // private static final String SEARCH_LIMIT_SQL = SEARCH_SQL + " LIMIT ?";
  //

  /** The Constant ALL_SEARCH_SQL. */
  private static final String ALL_SEARCH_SQL = "SELECT DISTINCT tags_samples_search.sample_id FROM tags_samples_search WHERE tags_samples_search.tag_keyword_search_id = ANY(?::int[])";
  ...
  // SAMPLE_KEYWORD_EXACT_SEARCH_CORE_SQL + "!= ?";

  /** The Constant SAMPLE_EXACT_SEARCH_CORE_SQL. */
  // private static final String SAMPLE_EXACT_SEARCH_CORE_SQL =
  // "SELECT tags_sample.sample_id FROM tags_sample WHERE tags_sample.tag_id = ?
  // AND tags_sample.value";
  ...
  public static List<SampleBean> searchSamples(JdbcTemplate jdbcTemplate,
    ...
    if (searchQueue.size() == 0) {
      ...
      // List<Integer> ids = getSampleIds(jdbcTemplate, tagId);
      // //getSampleIds(connection, tagKeywordIds);
  ...
  /**
   * Find all the samples matching keywords associated with a particular tag.
  ...
  /*
   * private static List<Integer> getTagKeywordIds(Connection connection, int
   * tagId, int keywordId) throws SQLException { return
   * Database.getIds(connection, TAG_KEYWORD_SQL, tagId, keywordId); }
   */
