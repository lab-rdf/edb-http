...
public abstract class DatabaseServlet  {
	...
	private static final String TAGS_SQL = 
			"SELECT tags.id, tags.name FROM tags WHERE tags.id = ANY(?::int[])";
	...
	protected static ResultsSetTable getTextTagsTable(Connection connection, 
		...
		PreparedStatement statement = 
				connection.prepareStatement(TAGS_SAMPLE_SQL);
	...
	protected static ResultsSetTable getTextTagTable(Connection connection, 
		...
		PreparedStatement statement = 
				connection.prepareStatement(SAMPLE_SPECIFIC_TAG_SQL);
	...
	protected static ResultsSetTable getIntTagsTable(Connection connection, 
		...
		PreparedStatement statement = 
				connection.prepareStatement(TAGS_SAMPLE_INT_SQL);
	...
	protected static ResultsSetTable getIntTagTable(Connection connection, 
		...
		PreparedStatement statement = 
				connection.prepareStatement(SAMPLE_SPECIFIC_INT_TAG_SQL);
	...
	protected static ResultsSetTable getFloatTagTable(Connection connection, 
		...
		PreparedStatement statement = 
				connection.prepareStatement(SAMPLE_SPECIFIC_FLOAT_TAG_SQL);
	...
	protected static ResultsSetTable getFloatTagsTable(Connection connection, 
		...
		PreparedStatement statement = 
				connection.prepareStatement(SAMPLE_SPECIFIC_FLOAT_TAGS_SQL);
	...
	protected static void processSample(Connection connection,
		...
		for (int i = 0; i < table.getRowCount(); ++i) {
			...
			//if (views.contains("all") || views.contains("samples")) {
			jsonArray.add(EDB.HEADING_ID, 
					sampleId);
			jsonArray.add(EDB.HEADING_EXPERIMENT, 
					experimentId);
			jsonArray.add(EDB.HEADING_TYPE, 
					table.getInt(i, 2));
			jsonArray.add(EDB.HEADING_NAME, 
					table.getString(i, 3));
			//sampleJSON.add(Application.HEADING_DESCRIPTION, table.getDataAsString(i, 4));
			jsonArray.add(EDB.HEADING_SPECIES, 
					table.getInt(i, 4));
			jsonArray.add(EDB.HEADING_DATE, 
					table.getString(i, 5));
	...
	public static void processSamples(Connection connection,
		...
		while (table.next()) {
			...
			Sample sample = new Sample(sampleId, 
					experiments.get(table.getInt(1)),
					expressionTypes.get(table.getInt(2)),
					table.getString(3),
					organisms.get(table.getInt(4)),
					date);
