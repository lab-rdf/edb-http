...
public class Samples {
  private static final String SAMPLE_FIELDS = "samples.id, samples.experiment_id, samples.expression_type_id, samples.name, samples.organism_id, TO_CHAR(samples.created, 'YYYY-MM-DD')";

  public static final String ALL_SAMPLES_SQL = "SELECT " + SAMPLE_FIELDS + " FROM samples";

  /**
   * Pick only samples we are
   */
  public static final String ALL_SAMPLES_WITHIN_GROUPS_SQL = "SELECT DISTINCT " + SAMPLE_FIELDS
  ...
  public static List<SampleBean> getSamples(final JdbcTemplate jdbcTemplate) throws SQLException {
    return jdbcTemplate.query(ALL_SAMPLES_SQL, new RowMapper<SampleBean>() {
      ...
      public SampleBean mapRow(ResultSet rs, int rowNum) throws SQLException {
        ...
        return new SampleBean(id, rs.getInt(2), rs.getString(4), rs.getInt(3), rs.getInt(5), rs.getString(6),
            sampleGroupIds);
  ...
  public static List<SampleBean> getSamples(JdbcTemplate connection, Collection<Integer> ids, int maxCount)
      throws SQLException {
   ...
   * @throws SQLException
   */
  public static String getSampleName(final JdbcTemplate connection, final int id) {
   ...
   * @throws SQLException
   */
  public static List<SampleBean> getSamples(final JdbcTemplate jdbcTemplate, final int id) throws SQLException {
    return Query.query(jdbcTemplate, Database.SAMPLE_SQL, id, new RowMapper<SampleBean>() {
      @Override
      public SampleBean mapRow(ResultSet rs, int rowNum) throws SQLException {

        Collection<Integer> sampleGroupIds = getGroups(jdbcTemplate, id);

        return new SampleBean(rs.getInt(1), rs.getInt(2), rs.getString(4), rs.getInt(3), rs.getInt(5), rs.getString(6),
            sampleGroupIds);
  ...
  public static SampleBean getSample(final JdbcTemplate jdbcTemplate, final int id) throws SQLException {
   ...
   * @return the sample
   */
  public static Sample getSample(Connection connection, int sampleId, Map<Integer, Experiment> experiments,
      Map<Integer, Species> organisms, TypeMap fieldMap, TypeMap expressionTypes, Map<Integer, Person> personMap) {
  ...
  public static List<PersonBean> getPersons(JdbcTemplate jdbcTemplate, int sid) throws SQLException {
    List<Integer> pids = Query.queryForIds(jdbcTemplate, SAMPLE_PERSON_IDS_SQL, sid);

    return Persons.getPersons(jdbcTemplate, pids);
  }

  public static List<GeoBean> getGeo(final JdbcTemplate jdbcTemplate, int sid) {
    return Query.query(jdbcTemplate, SAMPLE_GEO_SQL, sid, new RowMapper<GeoBean>() {
      @Override
      public GeoBean mapRow(ResultSet rs, int rowNum) throws SQLException {
        ...
        return new GeoBean(rs.getInt(1), Geo.getSeries(jdbcTemplate, seriesId), rs.getString(2),
   ...
   * @throws SQLException
   */
  public static Collection<Integer> getGroups(JdbcTemplate jdbcTemplate, int sampleId) throws SQLException {
   ...
   *           the SQL exception
   */
  public static ResultsSetTable getSamplesTable(Connection connection, Collection<Integer> ids, int maxCount)
      throws SQLException {
    if (maxCount > -1) {
